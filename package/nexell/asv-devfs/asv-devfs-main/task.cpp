...
const char *TaskParser::GetResultDir(void) const
{
	return m_ResultDir;
}
...
void TaskManager::SetData(void *Data)
{
	m_pData = Data;
}

void *TaskManager::GetData(void)
{
	return m_pData;
}
...
TASK_DESC_T *TaskManager::TaskGet(int index)
{
	int i = 0;

	for (auto t = m_TaskList.begin(); t != m_TaskList.end(); ++t, i++) {
		if (i == index)
			return (*t);
	}
...
bool TaskManager::TaskFailed(void)
{
	bool exit;
...
bool TaskManager::IsSuccessAll(void)
{
